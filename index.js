...
function firstNamesAllCaps(runners) {
  const runnersUpCase = runners.map(function(currentValue){
    return currentValue.first_name.toUpperCase();
  })
  return runnersUpCase;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const tShirtSizer = runners.filter(function(currentValue){
...
function tallyUpDonations(runners) {
  const tallyUper = runners.reduce(function(accumulator, currentValue){
 ...
 * etc
*/
function counterMakerWithLimit(lim) {
  let count = -1;
  return function() {
    if (count < lim){
      return count+= 1 ;
